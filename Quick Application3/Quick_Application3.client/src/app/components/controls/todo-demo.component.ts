...
export class TodoDemoComponent implements OnInit, OnDestroy {
  ...
  public _sortByImportant = false;
  public _sortByCompletion = false;
  ...
  toggleCompletion(row: Todo) {
    ...
    const index = this.rowsCache.findIndex(item => item.$$index === row.$$index);
    if (index !== -1) {
      this.rowsCache[index].completed = row.completed;
    }
...
toggleSortByCompletion() {
  this._sortByCompletion = !this._sortByCompletion;
  this.sortTasks();
  }
  toggleImportant(row: Todo) {
    row.important = !row.important;

    const index = this.rowsCache.findIndex(item => item.$$index === row.$$index);
    if (index !== -1) {
      this.rowsCache[index].important = row.important;
    }
  ...
  markAllCompleted() {
    this.rowsCache.forEach(task => task.completed = true);
    this.rows.forEach(task => task.completed = true);
  ...
  sortTasks() {
    let sortedRows = [...this.rowsCache];


    sortedRows.sort((a, b) => {
      if (this._sortByImportant) {
        if (a.important !== b.important) {
          return a.important ? -1 : 1;
        }
      }
      if (this._sortByCompletion) {
